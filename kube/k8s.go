...
// waitForJob waits for a signal from the jobEvents() that indicates that
//  the job status is set on active.
...
// waitForLogs waits for a signal from the podEvents() that indicates that the
// Pod status is set on Succeeded.
...
// jobEvents watches a job events. When the job status is active, a signal
// will be sent to the waitForJob() method.
func (k *K8s) jobEvents(job batchv1.Job, clientset *kubernetes.Clientset,
	namespace string, jobname string) error {
...
// podEvents watches a pods events. When the Pod status is Succeeded,
// a signal will be sent to the waitForLogs() method.
